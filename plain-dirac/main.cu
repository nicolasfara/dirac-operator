...
//  x = 2*(blockIdx.x * blockDim.x + threadIdx.x) + ((y+z+t) % 2);
  ...
  xm = x-1;
  xm = CHECK_PERIODIC_BOUNDARY_SUB(xm, nx);
  ym = y-1;
  ...
  xp = x+1;
  xp *= CHECK_PERIODIC_BOUNDARY_ADD(xp, nx);
  yp = y+1;
  ...
  // ETA can be ignored: computational purpose only. Not affect the simulation.
  eta = 1;
